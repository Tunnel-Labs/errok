...
export class ResultAsync<T, E> implements PromiseLike<Result<T, E>> {
	...
	static fromFunction<T, E>(
		asyncFunction: () => Promise<Result<T, E>>,
	): ResultAsync<T, E> {
		return new ResultAsync(asyncFunction());
	}

	static combinePromises<
		T extends readonly Promise<Result<unknown, unknown>>[],
	>(promises: T): CombineResultsAsync<{ [idx in keyof T]: Awaited<T[idx]> }> {
...
export type CombineResultsAsync<T extends readonly Result<unknown, unknown>[]> =
	IsLiteralArray<T> extends 1 ? Traverse<T> :
		ResultAsync<ExtractOkTypes<T>, ExtractErrTypes<T>[number]>;
...
type UnwrapAsync<T> = IsLiteralArray<T> extends 1 ?
	Writable<T> extends [infer H, ...infer Rest] ?
		H extends PromiseLike<infer HI> ?
			HI extends Result<unknown, unknown> ? [Dedup<HI>, ...UnwrapAsync<Rest>] :
			never :
		never :
	[] :
	...
	T extends Array<infer A> ?
		A extends PromiseLike<infer HI> ?
			HI extends Result<infer L, infer R> ? Ok<L, R>[] :
			never :
		never :
	never;
...
type TraverseAsync<T, Depth extends number = 5> = IsLiteralArray<T> extends 1 ?
	...
	T extends Array<infer I> ? // The MemberListOf<I> here is to include all possible types. Therefore
		// if we face (ResultAsync<X, Y> | ResultAsync<A, B>)[] this type should
		// handle the case.
		Combine<MemberListOf<I>, Depth> extends [infer Oks, infer Errs] ? // The following `extends unknown[]` checks are just to satisfy the TS.
			// we already expect them to be an array.
			Oks extends unknown[] ? Errs extends unknown[] ? ResultAsync<
