...
export interface TryUsageError<$Message> {
	_: $Message;
}
 ...
 */
export function $try<$Generator extends Generator>(
	body: (
...
export function $try<$AsyncGenerator extends AsyncGenerator>(
	body: (
		$ok: <T>(value: T) => Ok<T, never>,
		$err: <E>(error: E) => Err<never, E>,
	) => $AsyncGenerator,
...
> = [
	...
	T extends [infer H, ...infer Rest] ? // And test whether the head of the list is a result
		H extends Result<infer L, infer R> ? // Continue collecting...
			CollectResults<
	...
	T extends [] ? never :
		NeverArrayToNever extends 1 ?
			T extends [never, ...infer Rest] ?
				[EmptyArrayToNever<Rest>] extends [never] ? never :
				T :
			T :
		T;
...
export type IsLiteralArray<T> = T extends { length: infer L } ?
	L extends number ? number extends L ? 0 :
		1 :
	0 :
	0;
...
export type Traverse<T, Depth extends number = 5> = Combine<T, Depth> extends
	[infer Oks, infer Errs] ?
